...
def index():
    ...
    transactions_db = db.execute(
        "SELECT symbol, SUM(shares) AS shares, price FROM transactions WHERE user_id = ? GROUP BY user_id, symbol", user_id)
...
def buy():
    ...
    if request.method == "GET":
        ...
        if user_cash < transaction_value:
            return apology("Insuficent amount")
...
def login():
    ...
    if request.method == "POST":
        ...
        if int(len(rows)) != 1 or not check_password_hash(rows[0]["hash"], request.form.get("password")):
